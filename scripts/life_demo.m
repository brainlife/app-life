function [fh, pfe, dfe] = life_demo(dwiFile, t1File, ptckFileName, dtckFileName)
...
%  (they have assigned a zero-weight). Finaly, the root-mean-squared error
...
webwrite(getenv('SCA_PROGRESS_URL'), struct('msg','Importing .tck file into MATLAB','progress', 0), weboptions('MediaType','application/json'));

% import probabilistic tck file - will be overwritten in second half
fgFileName = dtiImportFibersMrtrix(ptckFileName);
...
pfe = feConnectomeInit(dwiFile,fgFileName,feFileName,[],dwiFileRepeat,t1File);
...
pfe = feSet(pfe,'fit',feFitModel(feGet(pfe,'mfiber'),feGet(pfe,'dsigdemeaned'),'bbnnls'));
...
prob.rmsexv = feGetRep(pfe,'vox rmse');
...
prob.rrmse  = feGetRep(pfe,'vox rmse ratio');
...
prob.w      = feGet(pfe,'fiber weights');
...
p.coords = feGet(pfe,'roi coords');

% keep pfe as return object
%clear pfe
...
% import deterministic tck file - replace existing fg
feFileName = dtiImportFibersMrtrix(dtckFileName);
...
dfe = feConnectomeInit(dwiFile,fgFileName,feFileName,[],dwiFileRepeat,t1File);

%% (2.2) Fit the model. 
% Hereafter we fit the forward model of tracrography using a least-squared
% method. The information generated by fitting the model (fiber weights
% etc) is then installed in the LiFE structure.
dfe = feSet(dfe,'fit',feFitModel(feGet(dfe,'mfiber'),feGet(dfe,'dsigdemeaned'),'bbnnls'));

%% (2.3) Extract the RMSE of the model on the fitted data set. 
% We now use the LiFE structure and the fit to compute the error in each
% white-matter voxel spanned by the tractography model.
det.rmse   = feGet(dfe,'vox rmse');

%% (2.4) Extract the RMSE of the model on the second data set. 
% Here we show how to compute the cross-valdiated RMSE of the tractography
% model in each white-matter voxel. We store this information for later use
% and to save computer memory.
det.rmsexv = feGetRep(dfe,'vox rmse');

%% (2.5) Extract the Rrmse. 
% We show how to extract the ratio between the model prediction error
% (RMSE) and the test-retest reliability of the data.
det.rrmse  = feGetRep(dfe,'vox rmse ratio');

%% (2.6) Extract the fitted weights for the fascicles. 
% The following line shows how to extract the weight assigned to each
% fascicle in the connectome.
det.w      = feGet(dfe,'fiber weights');
...
%% Extract the coordinates of the white-matter voxels.
% We will use this later to compare probabilistic and deterministic models.
d.coords = feGet(dfe, 'roi coords');

% keep dfe as return object
% clear dfe
